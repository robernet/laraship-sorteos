...
class Modules
{
    ...
    public function clearModulesSettingsCache()
    {
        ...
        \Cache::forget('module_settings_list_json');
    }
    ...
    {
        $json = Cache::remember('module_settings_list_json', config('corals.cache_ttl'), function () {
            $modules = [];
            ...
            foreach ($modules as $row) {
                ...
                array_push($modulesArray, $data);
            }

            return json_encode($modulesArray);
        });

        // Guard against old PHP-serialized cache entries
        $modules = is_string($json)
            ? collect(array_map(fn($item) => (object) $item, json_decode($json, true) ?? []))
            : collect([]);
